...
var pt_deptcd  = ""
var pt_docno = ""
let pt_recdate = receive_deptnm
let pt_rectime = receive_deptnm.replacingOccurrences(of:"\n      ", with:"")
...
class reserv_complete2 : UIViewController, XMLParserDelegate {
    ...
    override func viewDidLoad() {
        ...
        if(UserDefault.load(key: UserDefaultKey.UD_ClinicFlag) == "1"){
            pt_deptcd = UserDefault.load(key: UserDefaultKey.UD_ClinicDeptcd)
            pt_docno = UserDefault.load(key: UserDefaultKey.UD_ClinicDocno)
        } else if(plag_rec == 1  && reserve_index == 0){
    ...
    //    http_request(request_code: "reservesave")
        ...
        if(UserDefault.load(key: UserDefaultKey.UD_ClinicFlag) == "1"){
            rec_deptnm = UserDefault.load(key: UserDefaultKey.UD_ClinicDeptnm)
            rec_docnm = UserDefault.load(key: UserDefaultKey.UD_ClinicDocnm)
            
            resdate_LB.text = UserDefault.load(key: UserDefaultKey.UD_ClinicRsvDate)
        } else if(plag_rec == 1  && reserve_index == 0){
